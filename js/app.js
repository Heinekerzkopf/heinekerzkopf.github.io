(() => {
    ...
    items.forEach((item => {
        item.addEventListener("click", (e => {
            const currentlyActive = document.querySelectorAll(".spoller__item._spoller__active");
            if (currentlyActive) currentlyActive.forEach((active => {
                if (active !== item) {
                    active.lastElementChild.style.maxHeight = 0;
                    active.classList.toggle("_spoller__active");
                }
            }));
            item.classList.toggle("_spoller__active");
            const elementBody = item.querySelector(".spoller__body");
            if (item.classList.contains("_spoller__active")) elementBody.style.maxHeight = elementBody.scrollHeight + "px"; else elementBody.style.maxHeight = 0;
        }));
    }));
